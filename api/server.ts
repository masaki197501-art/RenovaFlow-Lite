...
async function createSharePointFolder(folderName: string) {
  ...
  const basePath = process.env.SHAREPOINT_BASE_PATH || "Renova";
  const sanitizedFolderName = folderName.replace(/[\\\/:*?"<>|]/g, '_');

  try {
    return await client.api(`/sites/${siteId}/drives/${driveId}/root:/${basePath}/${sanitizedFolderName}:`)
      .patch({
    ...
    console.error("SharePoint Folder Error:", e);
    return null;
  }
}
...
async function uploadFileToSharePoint(folderName: string, fileName: string, fileBuffer: Buffer) {
  ...
  try {
    return await client.api(`/sites/${siteId}/drives/${driveId}/root:/${basePath}/${sanitizedFolderName}/${fileName}:/content`)
      .put(fileBuffer);
  } catch (e) {
...
if (process.env.NODE_ENV !== "production") {
  ...
  app.post("/api/projects/:id/sharepoint-folder", async (req, res) => {
    const project = db.prepare("SELECT title, propertyName FROM projects WHERE id = ?").get(req.params.id);
    if (!project) return res.status(404).json({ error: "Project not found" });

    const folderName = project.propertyName || project.title;
    const result = await createSharePointFolder(folderName);
    
    if (result) {
      res.json({ success: true, folder: result });
    } else {
      res.status(500).json({ error: "SharePoint連携の設定を確認してください（環境変数が未設定、または権限不足の可能性があります）" });
    }
  });
